...
def train_full(module, epochs=300, seq_len=20, batch_size=256, lr=5e-4, patience=80):
    ...
    # Reduced capacity: [128,64] instead of [256,128,64]
    model = TemporalKANForecaster(input_dim, hidden_dims=[128, 64], output_dim=len(assets), grid_size=20)
    optimizer = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=1e-4)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=epochs)
    loss_fn = nn.HuberLoss(delta=1.0)  # more robust than MSE
...
def train_shrinking(module, start_year, epochs=300, seq_len=20, batch_size=256, lr=5e-4, patience=80):
    # Similar adjustments, omitted for brevity (same pattern as train_full)
    # For production, copy the same logic as train_full but with window filtering
    pass
