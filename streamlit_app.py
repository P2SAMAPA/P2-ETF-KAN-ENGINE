...
# FIX: do NOT include all 403 columns — options cols are NaN for historical rows.
REQUIRED_COLS  = FI_ASSETS + EQUITY_ASSETS + ['AGG', 'SPY'] + MACRO_COLS
...
def load_raw_data():
    """
    FIX: replaced load_dataset() + df.dropna() with hf_hub_download +
    pd.read_parquet + dropna(subset=REQUIRED_COLS).

    The old code used load_dataset() which auto-converts the parquet, then
    called df.dropna() on ALL 403 columns. Since the 191 options columns are
    entirely NaN for historical rows, every row was dropped → empty DataFrame
    → build_feature_sequence() returned None → "Insufficient data" warning.

    Fix: load the raw parquet directly and only drop rows where the price +
    macro columns (REQUIRED_COLS) are NaN. Options/ADV columns stay as NaN
    and are not used by build_feature_sequence().
    """
    token = get_hf_token()
    ...
    if "date" in df.columns:
        ...
        df.set_index("date", inplace=True)
        df.drop("__index_level_0__", axis=1, errors="ignore", inplace=True)
    ...
    # Forward-fill price/macro columns then drop only on required cols
...
def build_feature_sequence(df, module):
    ...
    for lag in range(1, 6):
        for a in assets:
            if a in df.columns:
                features[f'{a}_lag{lag}'] = df[a].ffill().pct_change(fill_method=None).shift(lag)
    # Drop NaNs only on the feature columns we just built
...
def load_model_and_scalers(module, mode='full', start_year=None):
    if mode == 'full':
        mf, sxf, syf = (f"kan_{module}_full.pt",
    ...
    model      = TemporalKANForecaster(input_dim, hidden_dims=[256, 128],
                                        output_dim=output_dim, grid_size=20,
                                        seq_len=SEQ_LEN)
    try:
        ...
        model.eval()
        metrics      = None
        metrics_path = download_file(
            f"metrics_{module}_shrinking_start{start_year}.pkl" if mode == 'shrinking'
            else f"metrics_{module}_full.pkl",
            "shrinking_models" if mode == 'shrinking' else "")
...
def get_weighted_shrinking_consensus_with_metrics(module, feature_seq, assets):
    ...
    patterns    = [f"shrinking_models/kan_{module}_shrinking_start",
    ...
    for mf in model_files:
        ...
        if model is None:
            all_metrics_display.append({'year': year, 'top_etf': 'N/A',
                'top_etf_return': None, 'status': 'Model Load Failed',
                'ann_ret': None, 'sharpe': None, 'max_dd': None,
        ...
        try:
            ...
            all_metrics_display.append({'year': year, 'top_etf': 'N/A',
                'top_etf_return': None, 'status': 'Prediction Failed',
                'ann_ret': None, 'sharpe': None, 'max_dd': None,
                'sharpe_capped': None, 'weight': 0, 'raw_score': 0})
            continue
        ...
        if metrics and 'test_predictions' in metrics and 'test_true' in metrics:
            ann_ret, sharpe, max_dd, hit, pred_var = compute_metrics_fixed(
                metrics['test_predictions'], metrics['test_true'])
        else:
        ...
        window_data.append({'year': year, 'prediction': pred, 'ann_ret': ann_ret,
            'sharpe': sharpe, 'sharpe_capped': sharpe_capped, 'max_dd': max_dd,
            'is_negative': is_negative, 'top_etf': top_etf,
            'top_etf_return': top_etf_return})
        all_metrics_display.append({'year': year, 'top_etf': top_etf,
            'top_etf_return': top_etf_return,
    ...
    for etf_idx in range(n_etfs):
        positive_count = sum(1 for w in valid_windows
                             if window_data[w]['prediction'][etf_idx] > 0)
        frequency_scores[etf_idx] = (positive_count / len(valid_windows) * 100
                                      if valid_windows else 0)

    final_window_scores = []
    for idx, w in enumerate(valid_windows):
        final_score = window_scores[w] + 0.10 * np.mean(frequency_scores)
...
def runners_html(assets, sorted_idx, pred):
    ...
    for rank, label in enumerate(["2nd", "3rd"], start=1):
        if len(sorted_idx) > rank:
            i = sorted_idx[rank]
            items += (f'<div class="runner-box">'
                      f'<div class="runner-label">{label}</div>'
                      f'<div class="runner-ticker">{assets[i]}</div>'
                      f'<div class="runner-return">{pred[i]*100:+.2f}%</div></div>')
...
def render_metrics_table(all_metrics, assets):
    ...
    for m in unique_metrics:
        ...
        sc           = m.get('sharpe_capped')
        sharpe_capped = (f" (capped: {sc:.1f})"
                         if sc and abs(sc) != abs(m['sharpe']) else "")
        ...
        top_ret      = (f"{m['top_etf_return']*100:+.2f}%"
                        if m.get('top_etf_return') is not None else "N/A")
        status_class = ("zero-weight" if "Excluded" in m['status']
                        else "valid-weight" if m['status'] == "Valid" else "")
        ann_class    = ("positive" if m['ann_ret'] and m['ann_ret'] >= 0
                        else "negative" if m['ann_ret'] and m['ann_ret'] < 0 else "")
        rows += (f'<tr class="{status_class}"><td>{m["year"]}</td>'
                 f'<td class="top-etf">{top_etf}</td><td>{top_ret}</td>'
                 f'<td>{m["status"]}</td><td class="{ann_class}">{ann_ret}</td>'
                 f'<td>{sharpe}<span class="sharpe-capped">{sharpe_capped}</span></td>'
                 f'<td>{max_dd}</td><td>{weight}</td></tr>')
    return (f'<table class="metrics-table">'
            f'<tr><th>Year</th><th>Top ETF</th><th>Top ETF Return</th>'
            f'<th>Status</th><th>Ann Return</th><th>Sharpe</th>'
            f'<th>Max DD</th><th>Weight</th></tr>{rows}</table>')
...
<p style="color:#666;font-size:0.9rem">
  Kolmogorov‑Arnold Network for ETF Return Forecasting · {now_str}</p>
</div>
""", unsafe_allow_html=True)
...
# ── Tabs ──────────────────────────────────────────────────────────────────────
tab_fi, tab_eq = st.tabs(["Option A — Fixed Income / Alts",
    ...
    with tab:
        ...
        with col_full:
            ...
            if model_full is None:
                render_html('<div class="hero-box not-available">'
                            '<div style="font-size:3rem;color:#ddd">◌</div>'
                            '<h3>Model not available</h3>'
                            '<p>Train via GitHub Actions (train.yml)</p></div>')
            else:
                ...
                switch_html = (f'<div class="switch-warning">⚠ Switched from {prev} · −12 bps</div>'
                ...
                meta        = (f'<div class="hero-meta">'
                               ...
                               f'Benchmark &nbsp;&nbsp;<strong>{benchmark}</strong></div>')
                pred_var    = np.var(pred)
                st.session_state.pred_var_full = pred_var
                debug_html  = f'<div class="debug-box">Pred variance: {pred_var:.6f}</div>'
                render_html(f'<div class="hero-box">'
                            f'<div style="font-size:0.85rem;color:#888;margin-bottom:0.3rem">'
                            f'Full Dataset · 2008–2026</div>'
                            f'<div class="hero-ticker">{top_asset}</div>'
                            f'<div class="hero-return">{display_ret*100:+.2f}% predicted</div>'
                            f'{switch_html}{meta}'
                            f'{runners_html(assets, sorted_idx, pred)}{debug_html}</div>')
        ...
        with col_cons:
            ...
            if cons_pred is None:
                render_html('<div class="hero-box not-available">'
                            ...
                            '<p>Train via GitHub Actions</p></div>')
            else:
                top_idx_c  = int(np.argmax(cons_pred))
                top_asset_c = assets[top_idx_c]
                sorted_c   = np.argsort(cons_pred)[::-1]
                meta_c     = (f'<div class="hero-meta">'
                              f'Signal for &nbsp;&nbsp;<strong>{next_day}</strong><br>'
                              f'Generated &nbsp;&nbsp;<strong>{now_str}</strong><br>'
                              f'Benchmark &nbsp;&nbsp;<strong>{benchmark}</strong></div>')
                consensus_html = (f'<div class="consensus-info">'
                                  f'{cons_info["valid_windows"]}/{cons_info["total_windows"]} '
                                  f'windows used · Sharpe capped at {MAX_SHARPE_FOR_WEIGHT}'
                                  f'</div>' if cons_info else '')
                ...
                render_html(f'<div class="hero-box">'
                            f'<div style="font-size:0.85rem;color:#888;margin-bottom:0.3rem">'
                            f'Weighted Shrinking Consensus</div>'
                            f'<div class="hero-ticker">{top_asset_c}</div>'
                            f'<div class="hero-return">'
                            f'{cons_pred[top_idx_c]*100:+.2f}% consensus</div>'
                            f'{meta_c}{consensus_html}'
                            f'{runners_html(assets, sorted_c, cons_pred)}{debug_html_c}</div>')

        # ── Shrinking metrics expander ────────────────────────────────────────
        if all_metrics and len(all_metrics) > 0:
            with st.expander(f"📊 Shrinking Windows Metrics ({module.upper()})",
                             expanded=False):
                st.markdown("**All years with top ETF pick and metrics:**")
                st.markdown("- 🟢 Green: Valid windows (positive return, included)")
                st.markdown("- 🔴 Red: Excluded (negative return, zero weight)")
                render_html(render_metrics_table(all_metrics, assets))
                st.markdown(f"""
**Weight Formula (Sharpe capped at {MAX_SHARPE_FOR_WEIGHT}):**
```
Score = (0.60 × AnnReturn) + (0.20 × (100 + MaxDD))
      + (0.10 × min(Sharpe, {MAX_SHARPE_FOR_WEIGHT}) × 10)
      + (0.10 × AvgFreq)
...
""")
        ...
        # ── Performance metrics ───────────────────────────────────────────────
        st.markdown('<h4 style="margin-top:1.5rem">Performance Metrics · Test Period</h4>',
                    unsafe_allow_html=True)
        metrics_data = load_metrics_full(module)
        if metrics_data:
            ann_ret, sharpe, max_dd, hit, pred_var = compute_metrics_fixed(
                metrics_data['test_predictions'], metrics_data['test_true'])
            var_warning = ('<br><span style="color:#d32f2f;font-size:0.75rem">'
                           '⚠ Low variance</span>' if pred_var < 0.0001 else "")
            render_html(
                f'<div class="metrics-grid">'
                f'<div class="metric-card"><div class="metric-label">Ann. Return</div>'
                f'<div class="metric-value" style="color:{"#2e7d32" if ann_ret>=0 else "#d32f2f"}">'
                f'{ann_ret:+.1f}%</div></div>'
                f'<div class="metric-card"><div class="metric-label">Sharpe Ratio</div>'
                f'<div class="metric-value" style="color:{"#2e7d32" if sharpe>=0 else "#d32f2f"}">'
                f'{sharpe:.2f}</div></div>'
                ...
                f'<div class="metric-value" style="color:{"#2e7d32" if hit>=50 else "#d32f2f"}">'
                f'{hit:.1f}%</div>{var_warning}</div></div>')
            with st.sidebar:
                st.metric(f"{module.upper()} Pred Var", f"{pred_var:.6f}")
        else:
            st.markdown('<div style="color:#999;font-style:italic">'
                        'Metrics not available</div>', unsafe_allow_html=True)

        # ── Signal history ────────────────────────────────────────────────────
        st.markdown('<h4 style="margin-top:1.5rem">Signal History</h4>',
                    unsafe_allow_html=True)
        if st.session_state.signal_history:
            ...
            st.markdown('<div style="color:#999;font-style:italic">'
                        'No signals recorded yet.</div>', unsafe_allow_html=True)

# ── Footer ────────────────────────────────────────────────────────────────────
st.markdown('<div style="text-align:center;color:#999;font-size:0.8rem;'
            'margin-top:2rem;padding-top:1rem;border-top:1px solid #eee">'
            'Research purposes only · Not financial advice</div>',
            unsafe_allow_html=True)
