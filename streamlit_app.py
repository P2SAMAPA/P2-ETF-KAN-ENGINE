...
for tab, module, assets, benchmark in [(tab_fi, 'fi', FI_ASSETS, FI_BENCHMARK),
    ...
    with tab:
        ...
        if model_full is None:
            st.warning(f"❌ Full model for {module} not found. Please run the GitHub Actions training workflow first.")
            st.info("Once training completes, the model will appear in the `models/` folder and predictions will be shown here.")
            continue   # <- No random numbers, just stop
